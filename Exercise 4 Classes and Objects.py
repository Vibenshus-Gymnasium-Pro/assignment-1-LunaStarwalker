...
class FeeSavingsAccount(SavingsAccount):
    ...
    def withdraw(self, pin, amount, fee):
        if self.amount_check(fee):
            self._balance -= fee
            if self.pin_check(pin) and self.amount_check(amount):
                self._balance -= amount
...
my_fee_savings = FeeSavingsAccount(1234)
...
my_fee_savings.deposit(1234, my_savings.balance(1234))
print(my_fee_savings.balance(1234))

my_fee_savings.withdraw(1234, 47621, 30)
print(my_fee_savings.balance(1234))